...
export default function Current({ weatherInfo, date, coordinates }) {
  ...
  useEffect(() => {
    setFavoris(context.user.favoris.findIndex(item =>
      item.city === context.locationInfo.city) !== -1 ?
      false : true)
  ...
  const movefavoris = async () => {
    ...
    const { city, country } = context.locationInfo

    if (favoris === true) {

      await addFavoris(coordinates.lng, coordinates.lat, city, country, token)
      ...
      await removeFavoris(city, token)
      let user = { ...context.user }
      user.favoris = user.favoris.filter(elem => elem.city.toUpperCase() !== city.toUpperCase())
      context.setUser(user)
      setFavoris(true)
    }
  }
